...
# Load vae and scaler
#----------
index_of_final_constraint = len(eq.CONSTRAINTS) - 1
vae_file = VAE_DIRECTORY + equation_name + '_v' + str(NUM_VARIABLES) + '_constraint' + str(index_of_final_constraint) + '_vae.pt'
n_dim = NUM_VARIABLES
...
try:
    ...
    exit('Error. VAE file does not exist at %s. Please make sure you run generate data and learn representation for each of the constraints' % vae_file)
vae.load_state_dict(vae_state_dict)

scaler_file = VAE_DIRECTORY + equation_name + '_v' + str(NUM_VARIABLES) + '_constraint' + str(index_of_final_constraint) + '_vae_scaler.pkl'
...
def optimiser_ga(run):
    ...
    if GENERATE_IMAGE:
        ...
        plt.savefig(IMAGE_DIRECTORY + equation_name + '_' + str(seed) + '_run' + str(run) + '_stackedcoil.png', dpi=72, bbox_inches='tight', pad_inches=0)
...
def main():
    ...
    results_file = RESULTS_DIRECTORY + equation_name + '_v' + str(NUM_VARIABLES) + '_stackedcoil' + '.pkl'    
