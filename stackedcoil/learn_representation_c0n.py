...
def load_data(directory, equation_name, constraint, debug):

    # load data for the specified constraint
    current_file = directory + equation_name + '_v' + str(NUM_VARIABLES) + '_constraint' + str(constraint) + '.pkl'
...
if __name__ == "__main__":
    ...
    #----------
    # Set constraint to determine which dataset to load
    #----------
    if args.constraint:
        constraint = int(args.constraint)
    else:
        exit("Error. Learning representation for multi-constraint problems. Please specify the constraint to generate vae for, first constraint has index 0")

    genomes, scaler = load_data(DATA_DIRECTORY, equation_name, constraint, debug)
    ...
    vae_file = VAE_DIRECTORY + equation_name + '_v' + str(NUM_VARIABLES) + '_constraint' + str(constraint) + '_vae.pt'
    vae.save(vae_file)
    print("VAE saved to", vae_file)

    scaler_file = VAE_DIRECTORY + equation_name + '_v' + str(NUM_VARIABLES) + '_constraint' + str(constraint) + '_vae_scaler.pkl'
