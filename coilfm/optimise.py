...
def calculate_objective_and_constraints(individual):
    ...
    item = vae.express(np_array_individual)
...
def optimiser_ga(run):
    ...
    while g < eq.VAEGA_GEN:
        ...
        if GENERATE_IMAGE:
            ...
            for ind in pop:
                ...
                individual = vae.express(np_array_individual)
    ...
    if GENERATE_IMAGE:
        ...
        plt.savefig(IMAGE_DIRECTORY + equation_name + '_' + str(seed) + '_run' + str(run) + '_disconcoil.png', dpi=72, bbox_inches='tight', pad_inches=0)
...
def main():
    ...
    results_file = RESULTS_DIRECTORY + equation_name + '_v' + str(NUM_VARIABLES) + '_disconcoil' + '.pkl'    
